...
generateColorizationData = 1;
SelectedNN = 360;
if generateColorizationData == 1
    ratio = 1;    % Image Resize Ratio.
    nnSize = 3;     % SP NN Size.
    stdSize = 5;    % Standard Deviation NN Size
    spCount = 2000; % Number of SP to be computed for each Image.

    inpColImg = cell(1, 1);
    InputImg = '.\input\mm3.png';
    inpColImg{1} = '.\input\mm\mm_col3.png';
%     inpColImg{2} = '.\input\mm\mm_col5.png';
    
%     inpColImg = cell(3, 1);
%     InputImg = '.\input\charlie.png';
%     inpColImg{1} = '.\input\charlie\dog.png';
%     inpColImg{2} = '.\input\charlie\wall.png';
%     inpColImg{3} = '.\input\charlie\people.png';

%     inpColImg = cell(2, 1);
%     InputImg = '.\input\monalisa.png';
%     inpColImg{1} = '.\input\monalisa\painting.png';
%     inpColImg{2} = '.\input\monalisa\monalisa_col.png';
    ...
    clc;disp('Computing SIFT for Input Image...');
    [inImg Sift1] = ComputeSIFT(InputImg, ratio);
%     [inImg Sift1] = ComputeSURF(InputImg, ratio);
    ...
    for I = 1:TotalSuperpixels
        ...
        tmp = Sift1 .* spTempArr;
        tmp = sum(sum(tmp, 1), 2)/sum(sp(:));
        spSIFT1(I, :) = reshape(tmp, 1, fcount);
        
        % Compute the mean Texture Value of each Super Pixel.
        tmp = texture1 .* spTempArr(:,:,1:size(texture1, 3));
        tmp = sum(sum(tmp, 1), 2)/sum(sp(:));
        spTexture1(I, :) = reshape(tmp, 1, size(texture1, 3));
    ...
    for counter = 1:size(inpColImg, 1)
        % Compute the SIFT features.
        clc;disp(['Computing SIFT for Color Image...' num2str(counter)]);
        [inCol Sift2] = ComputeSIFT(inpColImg{counter}, ratio); 
%         [inCol Sift2] = ComputeSURF(inpColImg{counter}, ratio); 
        % Compute the Turbo Pixels.
	...
	spTexture2 = zeros(TotalColorSuperPixels, size(texture1, 3));
    spCounter = 0;
    for counter = 1:size(inpColImg, 1)    
    ...
    save mm3_testdata_std_128.mat spTexture1 spTexture2 spStd1 spStd2 spSIFT1 spSIFT2 inImg inCol label1 label2 nnList1 nnList2 spCol2;
end
% clear all;
load mm3_testdata_std_128.mat
disp('computing NN...');
...
%   [costMatrix N] = FilterBasedOnIntensity(N, spStd1, spStd2, floor(nnCount/8), costMatrix, 0.5);
nnCount = floor(nnCount/8);
save tmp_lab.mat inImg inCol label1 label2 N costMatrix nnCount nnList1 spCol2;
test_median('tmp_lab.mat');
% test('island_lab.mat');
return;
